...
def fetch_products(wc_auth=None) -> list[dict]:
    ...
    if not products:
        log("  Using WordPress REST API + HTML scrape...", "yellow")
        products = _fetch_products_wp_api()

    if not products:
        log("  Falling back to full HTML crawl...", "yellow")
        products = _scrape_products_html()
...
def _discover_product_urls() -> list[str]:
        ...
        if r and "xml" in r.headers.get("content-type", ""):
            ...
            for loc in soup.find_all("loc"):
                ...
                if "/product/" in href or "/shop/" in href:
                    urls.add(href)
            if urls:
                log(f"  Found {len(urls)} URLs in sitemap", "dim")
                break

    # Crawl shop/category pages
    shop_pages = [f"{BASE_URL}/shop/", f"{BASE_URL}/indoor/", f"{BASE_URL}/outdoor/", f"{BASE_URL}/light-poles/"]
    for page_url in shop_pages:
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if "/product/" in href or "zionledusa.com/product" in href:
                urls.add(href if href.startswith("http") else BASE_URL + href)
        # Paginate
        for next_link in soup.select(".next, a[rel='next']"):
            next_url = next_link.get("href", "")
            r2 = safe_get(next_url)
            if r2:
...
def export_combined(categories, products, pages, meta):
    ...
    for p in products:
        ...
        (PROD_DIR / f"{s}.json").write_text(json.dumps(p, indent=2, ensure_ascii=False))
    ...
    export_path.write_text(json.dumps(combined, indent=2, ensure_ascii=False))
    ...
    (OUT_DIR / "products_index.json").write_text(json.dumps(index, indent=2, ensure_ascii=False))
